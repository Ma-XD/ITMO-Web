...
class HttpServer {
    ...
    private void process(Request request, Response response) throws IOException {
        ...
        if (file.isDirectory()) {
            String dirUri = request.getUri();
            file = new File(root + dirUri, DEFAULT_FILE);
        }

        if (file.isFile()) {
            byte[] body = readFile(file);
            String eTag = request.getHeader("If-None-Match");
            String currentETag = getFileETag(file, body);
    ...
    private String getFileETag(File file, byte[] content) {
        return Long.toString(file.hashCode() * 31L + Arrays.hashCode(content));
    }
    ...
    private static final class Request {
        ...
        private Request(String request) {
            ...
            uri = decodeUri(firstLineTokens[1]);
            headers = new HashMap<>();
        ...
        private  String decodeUri(String token) {
            try {
