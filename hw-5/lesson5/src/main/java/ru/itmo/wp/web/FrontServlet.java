...
import ru.itmo.wp.util.LanguageUtil;
...
public class FrontServlet extends HttpServlet {
    ...
    private static final Map<Class<?>, Object> EXPECTED_TYPE_TO_PARAMETER = new HashMap<>();
    ...
    private void process(Route route, HttpServletRequest request, HttpServletResponse response)
        ...
        EXPECTED_TYPE_TO_PARAMETER.put(Map.class, view);
        EXPECTED_TYPE_TO_PARAMETER.put(HttpServletRequest.class, request);
            ...
            for (Method clazzMethod : clazz.getDeclaredMethods()) {
                if (route.getAction().equals(clazzMethod.getName())) {
                    ...
                    for (Class<?> type : parameterTypes) {
                        if (!EXPECTED_TYPE_TO_PARAMETER.containsKey(type)) {
                            continue out;
                        }
                    }
                    method = clazzMethod;
        ...
        for (Class<?> type : parameterTypes) {
            parameters.add(EXPECTED_TYPE_TO_PARAMETER.get(type));
        }
        ...
        String lang = request.getParameter(LanguageUtil.KEY);
        HttpSession session = request.getSession();
        if (LanguageUtil.isLanguageParameter(lang)) {
            session.setAttribute(LanguageUtil.KEY, lang);
        }
        String pageName = LanguageUtil.getPageNameWithLanguage(
                session.getAttribute(LanguageUtil.KEY), pageClass.getSimpleName());

        Template template;
        try {
            template = newTemplate(pageName + ".ftlh");
